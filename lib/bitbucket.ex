defmodule Bitbucket do
  ...
  @user_agent [{"User-agent", "bitbucket-hex-package"}]

  @type response ::
          {:ok, Map.t(), HTTPoison.Response.t()}
  ...
  def process_response_body(body) do
    case Poison.decode(body, deserialization_options()) do
      {:ok, result} -> result
      {:error, invalid, _} -> raise invalid
      {:error, invalid} -> raise invalid
    end
  end
  ...
  Options:
    * `:pagination` - Can be `:none`, `:manual`, `:stream`, or `:auto`. Defaults to :auto
      ...
      of the time, this is what you want. For example, `Bitbucket.Repositories.list_users("chrismccord")`
      and `Bitbucket.Repositories.list_users("octocat")` have the same interface though one call
  ...
  def json_request(method, url, body \\ "", headers \\ [], options \\ []) do
    ...
    raw_request(method, url, encoded_body, headers, options)
  end
  ...
  def request_stream(method, url, auth, body \\ "", override \\ nil) do
    {:ok, encoded_body} = Poison.encode(body)
    request_with_pagination(method, url, auth, encoded_body)
  ...
  @doc """
  There is only one way to authenticate through Bitbucket API v2.0:

    * OAuth2 Token

  ## Examples

      iex> Bitbucket.authorization_header(%{access_token: "92873971893"}, [])
